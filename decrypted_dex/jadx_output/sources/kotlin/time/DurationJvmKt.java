...
public final class DurationJvmKt {
    ...
    private static final DecimalFormat createFormatForDecimals(int i3) {
    ...
    public static final String formatToExactDecimals(double d8, int i3) {
        ...
        if (i3 < threadLocalArr.length) {
            ThreadLocal<DecimalFormat> threadLocal = threadLocalArr[i3];
            DecimalFormat decimalFormat = threadLocal.get();
            if (decimalFormat == null) {
                decimalFormat = createFormatForDecimals(i3);
                threadLocal.set(decimalFormat);
            }
            createFormatForDecimals = decimalFormat;
        } else {
            createFormatForDecimals = createFormatForDecimals(i3);
        }
