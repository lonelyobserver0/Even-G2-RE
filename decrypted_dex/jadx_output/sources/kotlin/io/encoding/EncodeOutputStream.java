...
final class EncodeOutputStream extends OutputStream {
    ...
    public void write(byte[] source, int offset, int length) {
        ...
        checkOpen();
        if (offset < 0 || length < 0 || (i3 = offset + length) > source.length) {
            StringBuilder l9 = AbstractC1482f.l(offset, "offset: ", length, ", length: ", ", source size: ");
            l9.append(source.length);
        ...
        if (i10 < 3) {
            if (i10 != 0) {
                offset += copyIntoByteBuffer(source, offset, i3);
                if (this.byteBufferLength != 0) {
                    return;
                }
            }
            while (offset + 3 <= i3) {
                int min = Math.min((this.base64.getIsMimeScheme() ? this.lineLength : this.symbolBuffer.length) / 4, (i3 - offset) / 3);
                int i11 = (min * 3) + offset;
                if (encodeIntoOutput(source, offset, i11) != min * 4) {
                    throw new IllegalStateException("Check failed.");
                }
                offset = i11;
            }
            ArraysKt.copyInto(source, this.byteBuffer, 0, offset, i3);
            this.byteBufferLength = i3 - offset;
            return;
        }
        throw new IllegalStateException("Check failed.");
    }
