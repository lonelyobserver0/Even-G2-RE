...
public class SRP6Util {
    ...
    private static byte[] getPadded(BigInteger bigInteger, int i3) {
        byte[] asUnsignedByteArray = BigIntegers.asUnsignedByteArray(bigInteger);
        if (asUnsignedByteArray.length >= i3) {
            return asUnsignedByteArray;
        }
        byte[] bArr = new byte[i3];
        System.arraycopy(asUnsignedByteArray, 0, bArr, i3 - asUnsignedByteArray.length, asUnsignedByteArray.length);
        return bArr;
    }
    ...
    public static BigInteger validatePublicValue(BigInteger bigInteger, BigInteger bigInteger2) throws CryptoException {
        ...
        if (mod.equals(ZERO)) {
            throw new CryptoException(StubApp.getString2(28447));
        }
        return mod;
    }
}
