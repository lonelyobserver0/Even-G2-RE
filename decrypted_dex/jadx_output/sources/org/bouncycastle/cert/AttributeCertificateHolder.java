...
public class AttributeCertificateHolder implements Selector {
    ...
    public AttributeCertificateHolder(int i3, ASN1ObjectIdentifier aSN1ObjectIdentifier, ASN1ObjectIdentifier aSN1ObjectIdentifier2, byte[] bArr) {
        this.holder = new Holder(new ObjectDigestInfo(i3, aSN1ObjectIdentifier2, new AlgorithmIdentifier(aSN1ObjectIdentifier), Arrays.clone(bArr)));
    }
    ...
    private X500Name[] getPrincipals(GeneralName[] generalNameArr) {
        ...
        for (int i3 = 0; i3 != generalNameArr.length; i3++) {
            if (generalNameArr[i3].getTagNo() == 4) {
                arrayList.add(X500Name.getInstance(generalNameArr[i3].getName()));
            }
        }
