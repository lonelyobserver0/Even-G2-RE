...
public class CertPath {
    ...
    public CertPathValidationResult evaluate(CertPathValidation[] certPathValidationArr) {
        ...
        for (int i3 = 0; i3 != certPathValidationArr.length; i3++) {
            int length = this.certificates.length - 1;
            while (length >= 0) {
                try {
                    certPathValidationContext.setIsEndEntity(length == 0);
                    certPathValidationArr[i3].validate(certPathValidationContext, this.certificates[length]);
                } catch (CertPathValidationException e10) {
                    certPathValidationResultBuilder.addException(length, i3, e10);
                }
                length--;
            }
        }
        ...
        for (int i3 = 0; i3 != certPathValidationArr.length; i3++) {
            ...
            while (length >= 0) {
                try {
                    ...
                    return new CertPathValidationResult(certPathValidationContext, length, i3, e10);
                }
            }
        }
