...
public class GetCRL extends ASN1Object {
    ...
    private GetCRL(ASN1Sequence aSN1Sequence) {
        ...
        if (aSN1Sequence.size() > i3 && (aSN1Sequence.getObjectAt(i3).toASN1Primitive() instanceof ASN1GeneralizedTime)) {
            this.time = ASN1GeneralizedTime.getInstance(aSN1Sequence.getObjectAt(i3));
            i3++;
        }
        if (aSN1Sequence.size() <= i3 || !(aSN1Sequence.getObjectAt(i3).toASN1Primitive() instanceof ASN1BitString)) {
            return;
        }
        this.reasons = new ReasonFlags(ASN1BitString.getInstance(aSN1Sequence.getObjectAt(i3)));
    }
