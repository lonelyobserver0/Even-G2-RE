...
public class LDSSecurityObject extends ASN1Object implements ICAOObjectIdentifiers {
    ...
    private LDSSecurityObject(ASN1Sequence aSN1Sequence) {
        ...
        for (int i3 = 0; i3 < aSN1Sequence2.size(); i3++) {
            this.datagroupHash[i3] = DataGroupHash.getInstance(aSN1Sequence2.getObjectAt(i3));
        }
    }

    private void checkDatagroupHashSeqSize(int i3) {
        if (i3 < 2 || i3 > 16) {
            throw new IllegalArgumentException(StubApp.getString2(27118));
        }
    }
